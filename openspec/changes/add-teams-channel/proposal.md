...
- Polling-based message ingestion (not Graph subscriptions, which are unreliable) with cursor-based pagination
...
- **Auth**: Azure AD app registration required (client ID, client secret or certificate, tenant ID); tokens stored in `store/auth/{instanceName}/`
- **Credential proxy**: No changes needed — Teams tokens are resolved via existing `resolveSecretFile()` with instance-scoped paths
